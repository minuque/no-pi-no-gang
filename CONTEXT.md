...
- 连接断开后需要重连；重连时从 `lastEventSeq` 补齐缺失事件
...
| AgentRuntime | TaskSession | 当前的类 vs 规划中的治理抽象 |
...
- TaskSession 引入后，现有 `AgentSessionWrapper` 相关的 "session" 应逐步迁移为 "task"
